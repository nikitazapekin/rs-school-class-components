...
.required('Field is requered')
    ...
    .required('Field is requered')
        ...
        .required('Field is required')
        .test('is-valid-country', 'Invalid country selected', function(value) {
            const { options } = this; // Получаем доступ к контексту валидации
            const { countries } = options.context; // Получаем список стран из контекста
            return countries.includes(value); // Проверяем, что страна находится в списке
        }),
